...
function ValidationBadge({ status }: { status: StockValidation }) {
  const styles = {
    Tersedia: "bg-emerald-50 text-emerald-700 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:ring-emerald-500/20",
    "Stok Kurang": "bg-red-50 text-red-700 ring-red-200 dark:bg-red-500/10 dark:text-red-400 dark:ring-red-500/20",
    "Belum Dicek": "bg-gray-50 text-gray-700 ring-gray-200 dark:bg-zinc-800 dark:text-zinc-300 dark:ring-zinc-700",
  }

  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-semibold ring-1 ring-inset ${styles[status]}`}>
      {status}
    </span>
  )
}
...
export default function OutboundOrdersPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-zinc-50">Cek Order Keluar</h1>
      ...
      <section className="overflow-hidden rounded-lg border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
        <div className="border-b border-gray-200 dark:border-zinc-800 px-4 py-3">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-zinc-100">Daftar Order Outbound</h2>
            <p className="text-xs text-gray-600 dark:text-zinc-400">{orders.length} order</p>
          </div>
        </div>
          ...
          <table className="min-w-270 w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
              {orders.map((o) => {
                ...
                return (
                  <tr key={o.id} className="bg-white hover:bg-gray-50 dark:bg-zinc-950 dark:hover:bg-zinc-900 transition-colors">
                    <td className="px-4 py-3 font-semibold text-gray-900 dark:text-zinc-100">{o.id}</td>
                    <td className="px-4 py-3 text-gray-800 dark:text-zinc-300">{o.customer}</td>
                    <td className="px-4 py-3 text-gray-700 dark:text-zinc-400">{o.orderDate}</td>
                    <td className="px-4 py-3">
                      <div className="space-y-1">
                        {o.items.map((it) => (
                          <div key={`${o.id}-${it.sku}`} className="flex items-center justify-between gap-3">
                            <div className="min-w-0">
                              <div className="truncate font-semibold text-gray-900 dark:text-zinc-100">{it.sku}</div>
                              <div className="truncate text-xs text-gray-600 dark:text-zinc-400">{it.name}</div>
                            </div>
                            <div className="shrink-0 text-right text-sm font-semibold text-gray-900 dark:text-zinc-100">
                    ...
                    <td className="px-4 py-3 text-right">
                      {o.validation === "Belum Dicek" ? (
                        <button
                          ...
                          className="inline-flex items-center justify-center rounded-md border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-1.5 text-xs font-semibold text-gray-700 dark:text-zinc-300 shadow-sm hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors"
                        >
                          Cek Stok
                        </button>
                      ) : (
